...
test "TERMINAL record encodes native terminal state" {
    ...
    // A non-default palette owns an allocation rather than borrowing the
    // shared default. Encoding and decoding must preserve both its base
    // colors and overrides while giving each terminal independent ownership.
    var original_palette = terminal_color.default;
    original_palette[7] = .{ .r = 70, .g = 80, .b = 90 };
    try terminal.colors.palette.changeDefault(testing.allocator, original_palette);
    ...
    try testing.expect(terminal.colors.palette.original != restored.colors.palette.original);
    try testing.expectEqual(
        @as(usize, 12_345),
